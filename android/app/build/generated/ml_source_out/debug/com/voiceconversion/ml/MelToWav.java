...
public final class MelToWav {
  @NonNull
  private final Model model;

  private MelToWav(@NonNull Context context, @NonNull Model.Options options) throws IOException {
    model = Model.createModel(context, "mel_to_wav.tflite", options);
    MetadataExtractor extractor = new MetadataExtractor(model.getData());
  }

  @NonNull
  public static MelToWav newInstance(@NonNull Context context) throws IOException {
    return new MelToWav(context, (new Model.Options.Builder()).build());
  }

  @NonNull
  public static MelToWav newInstance(@NonNull Context context, @NonNull Model.Options options)
      throws IOException {
    return new MelToWav(context, options);
  }
